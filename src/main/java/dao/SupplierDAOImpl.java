...
public class SupplierDAOImpl implements SupplierDAO {
    ...
    private static final String INSERT_SUPPLIER = 
        "INSERT INTO suppliers (contact_name, company_name, email, phone_number, " +
        "address_line1, address_line2, city, state, postal_code, country, " +
        "website, supplier_type, is_active, created_at, updated_at) " +
        "VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)";
    
    private static final String SELECT_ALL_SUPPLIERS = 
        "SELECT * FROM suppliers ORDER BY company_name, contact_name";
    
    private static final String SELECT_SUPPLIER_BY_ID = 
        "SELECT * FROM suppliers WHERE id = ?";
    
    private static final String SELECT_SUPPLIERS_BY_CONTACT_NAME = 
        "SELECT * FROM suppliers WHERE LOWER(contact_name) LIKE LOWER(?) ORDER BY contact_name";
    
    private static final String SELECT_SUPPLIERS_BY_COMPANY_NAME = 
        "SELECT * FROM suppliers WHERE LOWER(company_name) LIKE LOWER(?) ORDER BY company_name";
    
    private static final String SELECT_SUPPLIERS_BY_TYPE = 
        "SELECT * FROM suppliers WHERE supplier_type = ? ORDER BY company_name";
    
    private static final String SELECT_ACTIVE_SUPPLIERS = 
        "SELECT * FROM suppliers WHERE is_active = true ORDER BY company_name, contact_name";
    
    private static final String SEARCH_SUPPLIERS = 
    ...
    private static final String SELECT_SUPPLIERS_WITH_PAGINATION = 
        "SELECT * FROM suppliers ORDER BY company_name, contact_name LIMIT ? OFFSET ?";
    
    private static final String UPDATE_SUPPLIER = 
        "UPDATE suppliers SET contact_name = ?, company_name = ?, email = ?, " +
        "phone_number = ?, address_line1 = ?, address_line2 = ?, city = ?, " +
        "state = ?, postal_code = ?, country = ?, website = ?, supplier_type = ?, " +
        "is_active = ?, updated_at = ? WHERE id = ?";
    
    private static final String UPDATE_SUPPLIER_STATUS = 
        "UPDATE suppliers SET is_active = ?, updated_at = ? WHERE id = ?";
    
    private static final String DELETE_SUPPLIER = 
        "DELETE FROM suppliers WHERE id = ?";
    
    private static final String COUNT_ALL_SUPPLIERS = 
        "SELECT COUNT(*) FROM suppliers";
    
    private static final String COUNT_SUPPLIERS_BY_TYPE = 
        "SELECT COUNT(*) FROM suppliers WHERE supplier_type = ?";
    
    private static final String CHECK_EMAIL_EXISTS = 
        "SELECT COUNT(*) FROM suppliers WHERE LOWER(email) = LOWER(?) AND id != ?";
    
    private static final String CHECK_COMPANY_NAME_EXISTS = 
        "SELECT COUNT(*) FROM suppliers WHERE LOWER(company_name) = LOWER(?) AND id != ?";
    ...
    public Supplier createSupplier(Supplier supplier) throws SQLException {
        ...
        try (PreparedStatement statement = connection.prepareStatement(INSERT_SUPPLIER, Statement.RETURN_GENERATED_KEYS)) {
    ...
    public List<Supplier> getAllSuppliers() throws SQLException {
        ...
        try (PreparedStatement statement = connection.prepareStatement(SELECT_ALL_SUPPLIERS);
    ...
    public Supplier getSupplierById(int id) throws SQLException {
        ...
        try (PreparedStatement statement = connection.prepareStatement(SELECT_SUPPLIER_BY_ID)) {
    ...
    public List<Supplier> getSuppliersByContactName(String contactName) throws SQLException {
        ...
        List<Supplier> suppliers = new ArrayList<>();
        try (PreparedStatement statement = connection.prepareStatement(SELECT_SUPPLIERS_BY_CONTACT_NAME)) {
    ...
    public List<Supplier> getSuppliersByCompanyName(String companyName) throws SQLException {
        ...
        List<Supplier> suppliers = new ArrayList<>();
        try (PreparedStatement statement = connection.prepareStatement(SELECT_SUPPLIERS_BY_COMPANY_NAME)) {
    ...
    public List<Supplier> getSuppliersByType(String supplierType) throws SQLException {
        ...
        List<Supplier> suppliers = new ArrayList<>();
        try (PreparedStatement statement = connection.prepareStatement(SELECT_SUPPLIERS_BY_TYPE)) {
    ...
    public List<Supplier> getActiveSuppliers() throws SQLException {
        ...
        List<Supplier> suppliers = new ArrayList<>();
        try (PreparedStatement statement = connection.prepareStatement(SELECT_ACTIVE_SUPPLIERS);
             ResultSet rs = statement.executeQuery()) {
    ...
    public List<Supplier> searchSuppliers(String keyword) throws SQLException {
        ...
        List<Supplier> suppliers = new ArrayList<>();
        try (PreparedStatement statement = connection.prepareStatement(SEARCH_SUPPLIERS)) {
    ...
    public List<Supplier> getSuppliersWithPagination(int offset, int limit) throws SQLException {
        ...
        List<Supplier> suppliers = new ArrayList<>();
        try (PreparedStatement statement = connection.prepareStatement(SELECT_SUPPLIERS_WITH_PAGINATION)) {
    ...
    public boolean updateSupplierStatus(int supplierId, boolean isActive) throws SQLException {
        ...
        try (PreparedStatement statement = connection.prepareStatement(UPDATE_SUPPLIER_STATUS)) {
    ...
    public int getTotalSupplierCount() throws SQLException {
        try (PreparedStatement statement = connection.prepareStatement(COUNT_ALL_SUPPLIERS);
             ResultSet rs = statement.executeQuery()) {
    ...
    public int getSupplierCountByType(String supplierType) throws SQLException {
        try (PreparedStatement statement = connection.prepareStatement(COUNT_SUPPLIERS_BY_TYPE)) {
            statement.setString(1, supplierType);
    ...
    public boolean isCompanyNameExists(String companyName, int excludeId) throws SQLException {
        try (PreparedStatement statement = connection.prepareStatement(CHECK_COMPANY_NAME_EXISTS)) {
            statement.setString(1, companyName);
            statement.setInt(2, excludeId);
